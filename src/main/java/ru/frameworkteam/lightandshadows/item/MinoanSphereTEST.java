...
public class MinoanSphereTEST extends Item {
    public MinoanSphereTEST(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        if (charge) {
            ...
            if (player.isSneaking() && !stack.contains(ModComponents.POSITION_COMPONENT)) {
                ...
                LightAndShadows.LOGGER.info("Pos1: " + position);
                LightAndShadows.LOGGER.info("Pos2: " + stack.get(ModComponents.POSITION_COMPONENT));

            } else if (stack.contains(ModComponents.POSITION_COMPONENT)) {

                stack.set(ModComponents.CHARGE_COMPONENT, false);

                PositionComponent position = stack.get(ModComponents.POSITION_COMPONENT);
//                LightAndShadows.LOGGER.info(position.toString());

              player.teleport(position.x(), position.y(), position.z(), false);

                stack.remove(ModComponents.POSITION_COMPONENT);
            }
        }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        if (stack.contains(ModComponents.POSITION_COMPONENT)) {
            ...
            if (Screen.hasShiftDown()) {
                ...
                String positionX = decimalFormat.format(position.x());
                tooltip.add(Text.of("§7X: §6" + positionX));
                String positionY = decimalFormat.format(position.y());
                tooltip.add(Text.of("§7Y: §6" + positionY));
                String positionZ = decimalFormat.format(position.z());
                tooltip.add(Text.of("§7Z: §6" + positionZ));

                String dimensionID = position.dimension();
